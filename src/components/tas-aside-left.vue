<template>
  <div class="aside aside-left aside-fixed d-flex flex-column flex-row-auto" id="kt_aside"
    ...
    <div class="aside-menu-wrapper flex-column-fluid" id="kt_aside_menu_wrapper">
      ...
      >
        <!-- {{ `Active: ${asideActiveId}` }} {{ `height: ${windowHeight}` }} -->
        <!--begin::Menu Nav-->
        <perfect-scrollbar
          class="aside-menu scroll"
          style="position: relative;"
          :style="'height: '+(windowHeight-(65*2))+'px'"
        >
          <ul class="menu-nav">
                      ...
                      <div class="menu-submenu">
                        ...
                        <ul class="menu-subnav">
                          ...
                          <li
                            class="menu-item"
                            aria-haspopup="true"
                            v-for="(item3, index) in item2.items"
                            :key="`${item3.id}-${index}-sub-menu`"
                            v-bind:class="{
                              'menu-item-open': hasActiveChildren(item3.to) || asideActiveId == item3.id || asideActiveId.toString().includes(item3.id),
                              'menu-item-active': $route['path'] == item3.to,
                            }"
                          >

                            <!-- MENU LV 3 -->
                            <router-link :to="item3.to ? item3.to : '/'" class="menu-link" @click="activate(item3.id)" v-if="item3.isMenu && !item3.hasSub">
                              <i class="menu-bullet menu-bullet-dot">
                                <span></span>
                              </i>
                            ...
                            <a
                              ...
                              class="menu-link menu-toggle"
                              v-if="item3.hasSub"
                              @click="togleSecondary(true); activate(item3.id)"
                            >
                              <i class="menu-bullet menu-bullet-line">
                                <span></span>
                              </i>
                            ...
                            <div class="menu-submenu">
                              ...
                              <ul class="menu-subnav">
                                <li
                                  class="menu-item"
                                  aria-haspopup="true"
                                  v-for="(item4, index) in item3.items"
                                  v-bind:class="{
                                    'menu-item-open': hasActiveChildren(item4.to) || asideActiveId == item4.id || asideActiveId.toString().includes(item4.id),
                                    'menu-item-active': $route['path'] == item4.to,
                                  }"
                                  :key="`${item4.id}-${index}-sub-menu`"
                                >
                                  <router-link :to="item4.to ? item4.to : '/'" class="menu-link" @click="activate(item4.id)">
                                    <i class="menu-bullet menu-bullet-dot">
                                      <span></span>
                                    </i>
                                    <span class="menu-text">{{ item4.title }}</span>
                                  </router-link>
                                </li>
        ...
        <div class="pt-3 px-3">
          <div class="d-flex w-100" style="gap: 10px;">
            <a @click="logOut()" class="btn btn-light-dark btn-hover-danger font-weight-bold btn-md mt-0 btn-block" style="color: #b5b5c3; background-color: rgba(63,66,84,.35)">Keluar</a>
            <!-- <a @click="reloadModule()" class="btn btn-light-dark btn-hover-danger font-weight-bold btn-md mt-0 btn-block text-white" style="color: #b5b5c3; background-color: rgba(63,66,84,.35)">
              <i class="ri-restart-line"></i>
            </a> -->
            <a @click="reloadModule()" class="btn btn-light-danger font-weight-bold mr-2">
              <i class="ri-restart-line px-1"></i>
            </a>
...
export default {
  ...
  methods: {
    ...
    getMenu () {
      ...
      if (menu) this.buildMenu(JSON.parse(menu))
      else {
        this.$_api['get']('/menu/list/build', {})
          .then((res) => {
            localStorage.setItem('menu', JSON.stringify(res.data[0]))
            this.buildMenu(res.data[0])
          })
          .catch(() => {
    ...
    buildMenu (data) {
      let menu = []
      for (const item of data._children) {
        let menu2 = []
          ...
          for (const item2 of item._children) {
            ...
            if (item2._children.length) {
              for (const item3 of item2._children) {
                ...
                let menu4 = []
                if (item3._children.length) {
                  for (const item4 of item3._children) {
                    menu4.push({
                      id: item4.code,
                      title: item4.name,
                      to: item4.to,
                      isMenu: true,
                      hasSub: !!item4._children.length,
                      items: []
                    })
                  }
                }
    ...
    createLink (s) {
      console.log('===> tas-aside-left.vue:470 ~ ', );
