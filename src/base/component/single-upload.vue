<template>
  <div class="img-form-wrapper">
    <div class="input-img">
      <label>
        <a>
          <img v-if="uploadURL" v-img-fb="(uploadURL || uploadURL.url) || './static/img/default/camera.svg'" alt="">
...
export default {
  ...
  watch: {
    value: {
      ...
      immediate: true,
      handler (to) {
        if (typeof to === 'object') this.uploadURL = Object.assign({}, this.uploadURL, to)
        else if (typeof to === 'string' && to) {
          this.uploadURL = { 
            "url": `${this.urlBe}/${to}`,
            "tumbnail_url": `${this.urlBe}/${to}`,
            "filename": to,
            "path": to,
            "ext": to
          }
