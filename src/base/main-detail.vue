<template>
  <div class="detail-wrapper">
    <slot v-if="isBodySlot" name="detail-body" v-bind:rowData="realData" />
    <template v-else>
      <div class="d-flex mb-5">
        ...
        <div class=" flex-fill">
          <div v-if="!isLoading" class="card card-custom">
            <div class="card-header border-0 pt-5 pb-5" style="min-height:0">
              ...
              <div class="card-toolbar my-0">
                <a  @click="emitEvent('back')" class="btn btn-outline-primary btn-icon btn-circle" v-b-tooltip.top="$t('global.go_back')">
                  <i class="ri-arrow-go-back-line p-0"></i>
                </a>
              </div>
            </div>
            <div class="card-body py-2 mt-3" v-if="fields && fields.length > 0">
                            ...
                            <template v-for="(v, k) in mainData[h.id]">
                              <template v-if="(v.data instanceof Array)">
                                ...
                                <template v-else-if="v.type === 'file'">
                                  <template v-for="(d, l) in v.data">
                                    <img :key="k+'-value-multiple-'+l" class="img-table " :class="classReader(d, h.class)" v-img-fb="urlBe+'/'+d" :alt="h.id" @click="emitEvent('zoom', d)" />
                                  </template>
                                </template>
                                <template v-else>
                              ...
                              <template v-else>
                                ...
                                <!-- <template v-else-if="v.type === 'file'"> -->
                                <img v-else-if="v.type === 'file'" :key="k+'-value'" class="img-table" :class="classReader(d, h.class[k])" v-img-fb="urlBe+'/'+v.data" :alt="h.id" @click="emitEvent('zoom', v.data)" />
                                <!-- </template> -->
                          ...
                          <template v-else>
                            <template v-if="(mainData[h.id].data instanceof Array)">
                              ...
                              <template v-else-if="mainData[h.id].type === 'file'">
                                <template v-for="(d, k) in mainData[h.id].data">
                                  <img :key="k+'-value'" class="img-table " :class="classReader(d, h.class)" v-img-fb="urlBe+'/'+d" :alt="h.id" @click="emitEvent('zoom', d)" />
                                </template>
                              </template>
                              <template v-else>
                            ...
                            <template v-else>
                              ...
                              <a style="word-break: break-all;" v-else-if="mainData[h.id].type === 'link'" class="text-primary " :class="[classReader(mainData[h.id].data, h.class)]" :href="mainData[h.id].data" target="_blank">{{ mainData[h.id].data | parse(h.transform) }}</a>
                              <template v-else-if="mainData[h.id].type === 'file'">
                                <div class="d-flex justify-content-between">
                                  <p class="text-dark font-style-underline pointer">{{ realData[h.id] }}</p>
                                ...
                                <!-- <template v-if="realData[h.id]">
                                  <template v-if="supportDisplayExt.includes(realData[h.id].split('.').pop())">
                                    <div class="d-flex justify-content-between">
                                  ...
                                  <template v-if="supportDisplayInt.includes(realData[h.id].split('.').pop())">
                                    <div class="d-flex justify-content-between">
                                      <a @click="toggleIframeFile(mainData[h.id])" class="text-primary font-style-underline pointer">{{ showIframe.includes(mainData[h.id])? 'Sembunyikan' : 'Lihat' }} {{ realData[h.id].filename }}</a>
                                      <a :href="mainData[h.id].data" download target="_blank" class="label label-info label-inline pointer">Download</a>
                                    </div>
                                  ...
                                  <img v-else class="img-table " :class="classReader(mainData[h.id].data, h.class)" v-img-fb="urlBe+'/'+mainData[h.id].data" :alt="h.id" @click="emitEvent('zoom', mainData[h.id].data)" />
                                </template>
                                <img v-else class="img-table " :class="classReader(mainData[h.id].data, h.class)" v-img-fb="urlBe+'/'+mainData[h.id].data" :alt="h.id" @click="emitEvent('zoom', mainData[h.id].data)" /> -->
                              </template>
                              <template v-else-if="mainData[h.id].type === 'image'">
                                <img v-if="mainData[h.id]" v-img-fb="urlBe+'/'+mainData[h.id].data" alt="" class="img-thumbnail max-w-100px">
    ...
    <div v-if="slaveList.length!==0 && activeSlave">
      ...
      <div v-if="activeSlave" class="mt-5">
