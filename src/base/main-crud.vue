<template>
  <div :class="[slave ? 'container-fluid p-0' : classes || 'container-fluid']">
    <template v-if="!isLoading">
      <div id="create" ref="create" class="animated" v-if="activeView === 'create'">
        <slot name="create-form">
          <tas-base-form :slave="slave"
            @back="config.redirectAfterForm ? onSuccessForm('create', null) : getEvent('list')"
            @success="onSuccessForm('create', $event)" :key="config.title + '-form-create'" :form="'create'"
            :API="{ setter: configAPI.create, getter: configAPI.read }" :title="config.title" :fields="fields.create">
      ...
      <div id="update" ref="update" class="animated" v-if="activeView === 'update'">
        <slot name="update-form" v-bind:data="{ 'id': activeID, 'data': activeData }">
          <tas-base-form :slave="slave" :custom-getter="customGetter"
            @back="config.redirectAfterForm ? onSuccessForm('update', null) : getEvent('list')"
            @success="onSuccessForm('update', $event)" :key="config.title + '-form-update'" :id="activeID"
            :form="'update'" :API="{ setter: configAPI.create, getter: configAPI.read }" :title="config.title"
            :fields="fields.update" :pkField="pkField">
            <template v-slot:form-header="rowData">
              <slot name="form-header" v-bind:rowData="rowData.rowData"></slot>
            </template>
          </tas-base-form>
      ...
      <div id="detail" ref="detail" class="animated" v-if="activeView === 'detail'">
        ...
        <tas-base-detail :isBodySlot="!!$scopedSlots['detail-body']"
          :isSideSlot="!!$scopedSlots['detail-side-right-body'] || !!$scopedSlots['detail-side-left-body']"
          :isCustomDelete="config.custom_delete ? !!config.custom_delete : false"
          :isCustomAPI="config.custom_api ? !!config.custom_api.read : false" @zoom="getEvent('zoom', $event)"
          @back="getEvent('list')" :key="config.title + '-detail'" :id="activeID" :slave="config.slave"
          :permission="permission" :title="config.title" :API="configAPI.read" :fields="fields.detail"
      ...
      <div id="list" ref="list" class="animated" v-if="activeView === 'list'">
        ...
        <slot name="list-body">
          <!-- {{ config.filter_api }} -->
          <tas-base-table ref="base-table-default" :isTableSlot="!!$scopedSlots['header-table'] || !!$scopedSlots['list-table']"
            :hideColumnVisibility="hideColumnVisibility" :hideFilter="hideFilter" :hideGlobalSearch="hideGlobalSearch"
            :filterView="filterView" :headerFilterView="headerFilterView" v-if="!listCard"
            @zoom="getEvent('zoom', $event)" @add_new="getEvent('create')" @filter="getEvent('filter')"
            @export="getEvent('export')" @import="getEvent('import')" @detail="getEvent('detail', $event)"
            @update="getEvent('update', $event)" @delete="getEvent('delete', $event)" @getMainData="handleGetMainData"
            @updateFilters="handleUpdateFilters" :key="config.title + '-list'" :slave="slave" :permission="permission"
            :title="config.title" :API="configAPI.list"
            :isCustomDelete="config.custom_delete ? !!config.custom_delete : false"
            :isCustomAPI="config.custom_api ? !!config.custom_api.list : false" :endpoint="configAPI"
            :fields="fields.list" :export="config.export" :import="config.import" :filters="{ ...config.filter_api, ...filters }"
            :frontendPaginate="config.frontendPaginate" :pkField="pkField"
            :customDetailPagination="customDetailPagination" :optionalProps="config.optionalProps">
            <template v-if="!advanceFilter" v-slot:inline-filter>
              <tas-base-form class="animated" btnSaveLabel="Terapkan Filter" v-show="filterView" @back="getEvent('list')"
                @success="onSuccessForm('filter', $event)" :key="config.title + '-inline-form-filter'" simple
                :form="'filter'" :API="{ setter: configAPI.create, getter: configAPI.read }" :title="config.title"
                :fields="fields.filter" />
            </template>
            <template v-if="headerFilterView" v-slot:header-filter>
              <slot name="header-filter"></slot>
            </template>
            <template v-slot:top-table>
              <slot name="top-table" v-bind:data="{filters: filters}"></slot>
            </template>
            <template v-slot:table-action>
              <slot name="list-action"></slot>
            </template>
            <template v-slot:table-action-append>
              <slot name="list-action-append"></slot>
            </template>
            <template v-slot:list-table="rowData">
              <slot name="list-table" v-bind:rowData="rowData.rowData"></slot>
            </template>
            <template v-slot:subtitle="data">
              <slot name="subtitle" v-bind:data="data.data.data"></slot>
            </template>
            ...
            <template v-slot:header-table="rowData">
              <slot name="header-table" v-bind:rowData="rowData.rowData"></slot>
            </template>
            <template v-slot:footer-table>
              <slot name="footer-table"></slot>
            </template>
          </tas-base-table>
          <tas-base-card v-else @zoom="getEvent('zoom', $event)" @add_new="getEvent('create')"
            @filter="getEvent('filter')" @export="getEvent('export')" @import="getEvent('import')"
            @detail="getEvent('detail', $event)" @update="getEvent('update', $event)" @delete="getEvent('delete', $event)"
            :key="config.title + '-list'" :slave="slave" :permission="permission" :title="config.title"
            :API="configAPI.list" :endpoint="configAPI" :fields="fields.list" :export="config.export"
            :import="config.import" :filters="{ ...config.filter_api, ...filters }" :frontendPaginate="config.frontendPaginate">
            <template v-if="!advanceFilter" v-slot:inline-filter>
              <tas-base-form class="animated" v-show="filterView" @back="getEvent('list')"
                @success="onSuccessForm('filter', $event)" :key="config.title + '-inline-form-filter'" simple
                :form="'filter'" :API="{ setter: configAPI.create, getter: configAPI.read }" :title="config.title"
                :fields="fields.filter" />
            </template>
    ...
    <template v-else>
      <div class="h-90vh text-center position-relative animated">
        <img src="/static/img/loader.svg" class="img-fluid"
          style="transform: translate(-50%, -50%);position: absolute;top: 50%;">
      </div>
    </template>
    ...
    <b-sidebar v-if="!isLoading && advanceFilter" @hidden="filterView = false" :visible="filterView" id="filter-component"
      backdrop :backdrop-variant="'light'" shadow right no-header body-class="bg-white">
      <tas-base-form btnSaveLabel="Terapkan Filter" @back="getEvent('list')" @success="onSuccessForm('filter', $event)"
        :key="config.title + '-form-filter'" :form="'filter'" :API="{ setter: configAPI.create, getter: configAPI.read }"
        :title="config.title" :fields="fields.filter" />
    </b-sidebar>
  </div>
</template>
<script>
...
export default {
  ...
  props: {
    ...
    hideFilter: { type: Boolean, required: false, default() { return true } },
    ...
    mockSlave: { type: Boolean, required: false, default() { return false } },
  ...
  computed: {
    slave() {
      if (!this.mockSlave) {
        ...
        let tmpSlave = {
          name: 'mock-slave',
          master: this.config.title,
          rules: []
        }
        this.$store.commit('set', ['activeSlave', tmpSlave, true])
        return true
      }
    }
  },
  watch: {
    config: {
      ...
      immediate: true,
      handler(to, from) {
        this.moduleChange(to)
        // if (this.slave && this.$store.state.activeSlave) {
        //   if (this.$store.state.activeSlave.rules) {
  ...
  methods: {
        ...
        if (item.methods) {
          ...
          if (item.methods.create) {
            if (typeof item.methods.create === 'object') {
              create = {
                conditionalShow: item.conditionalShow,
    ...
    swapPrimaryField(arr, primary) {
      if (primary) this.pkField = primary
    ...
    onSuccessForm(from, e) {
      if (from === 'update') {
        ...
        this.$emit('after-create', e)
      } else if (from === 'filter') {
        this.filters = { ...this.config.filter_api, ...this.$_sys.cleansingObj(e) }
        if(this.$refs && this.$refs['base-table-default']) this.$refs['base-table-default'].getData(this.filters)
        this.filterView = true
      }
    },
    getEvent(type, data = null) {
      // Kadang ada yang id_{modul} ada juga yang {modul}_id
      let snake_case_id = this.config.getter.replace(/[A-Z]/g, letter => `_${letter.toLowerCase()}`) + '_id'
      let snake_case_id_reverse = 'id_' + this.config.getter.replace(/[A-Z]/g, letter => `_${letter.toLowerCase()}`)
      // console.log('getEvent', type, this.config.filter_api)

      // this.params = this.config && this.config.filter_api ? { ...this.config.filter_api } : {}
      switch (type) {
        case 'detail':
          this.activeView = 'detail'
          this.activeID = data[snake_case_id] ? data[snake_case_id] : (data[snake_case_id_reverse] ? data[snake_case_id_reverse] : data.id)
