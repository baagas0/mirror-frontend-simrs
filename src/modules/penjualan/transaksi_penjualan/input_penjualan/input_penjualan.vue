<template>
    <div>
                    ...
                    <b-col cols="3">
                      <div class="mt-1 mb-2 mr-2">
                        <div class="btn-group w-100">
                          <b-button 
                            class="btn" 
                            :variant="dataHeader.is_external === null || dataHeader.is_external === false ? 'success' : 'secondary'" 
                            size="sm"
                            :disabled="dataHeader.is_external === true || headerBusy || isDraft > 0"
                            @click="$bvModal.show('modal-pasien')"
                          >internal</b-button>
                          <b-button 
                            class="btn" 
                            :variant="dataHeader.is_external === null || dataHeader.is_external === true ? 'primary' : 'secondary'" 
                            size="sm"
                            :disabled="dataHeader.is_external === false || headerBusy || isDraft > 0"
                            @click="$bvModal.show('modal-pasien-external')"
                          >external</b-button>
                      ...
                      <div class="mt-3">
                        ...
                        <h5 class="ml-2 my-auto">
                          {{dataHeader.no_rm || '-'}} / {{dataHeader.no_kunjungan || '-'}}
                        </h5>
                      </div>
                      <!-- <div class="mt-2">
                      ...
                      <b-row style="row-gap: 15px;" class="mx-1">
                        ...
                        <b-col cols="6">
                          ...
                          <date-picker
                            ...
                            v-model="dataHeader.tgl_penjualan"
                            :state="checkValidasi('tgl_penjualan').s"
                        ...
                        <b-col cols="4">
                          ...
                          <multiselect
                            id="jenis_layanan"
                            v-model="dataHeader.jenis_layanan"
                            ...
                            size="sm"
                            :disabled="dataHeader.registrasi_id != null || headerBusy || isDraft > 0"
                            :class="{invalid: !checkValidasi('jenis_layanan').b}"
                          ></multiselect>
                        ...
                        <b-col cols="4">
                          ...
                          <multiselect
                            ...
                            :show-labels="false"
                            label="nama_dokter"
                            track-by="ms_dokter_id"
                            placeholder="-- DPJP --"
                            size="sm"
                            :class="{invalid: !checkValidasi('dokter').b}"
                            :disabled="headerBusy || isDraft > 0"
                          ></multiselect>
                        ...
                        <b-col cols="4">
                          ...
                          <multiselect
                            ...
                            placeholder="-- Gudang --"
                            size="sm"
                            :class="{invalid: !checkValidasi('gudang').b}"
                            :disabled="headerBusy || isDraft > 0"
                          ></multiselect>
                        ...
                        <b-col cols="4">
                          ...
                          <multiselect
                            id="kelas"
                            v-model="dataHeader.kelas_kunjungan"
                            ...
                            placeholder="-- Kelas --"
                            size="sm"
                            :class="{invalid: !checkValidasi('kelas_kunjungan').b}"
                            :disabled="dataHeader.registrasi_id != null || headerBusy || isDraft > 0"
                          ></multiselect>
                        ...
                        <b-col cols="4">
                          ...
                          <b-form-input
                            id="no_asuransi"
                            v-model="dataHeader.no_asuransi"
                            disabled
                            :state="checkValidasi('no_asuransi').s"
                          ></b-form-input>
                    ...
                    <b-row>
                      ...
                      <b-col cols="6">
                        <Perhitungan 
                          :dataHeader="dataHeader" 
                          :dataPerhitungan="dataPerhitungan" 
                          :listJualPenunjang="listJualPenunjang" 
                          :listJualFasilitas="listJualFasilitas" 
                          :listJualJasa="listJualJasa" 
                          :listJualBarang="listJualBarang" 
                          :isDraft="isDraft"
                          :statusTagihan="statusTagihan"
                          :headerBusy="headerBusy"
                          :isValid="isValid"
                          :isDirty="isDirty"
                          @touchForm="v$.dataHeader.$touch()"
                          @changeVar="changeVar"
                          @resetData="resetData"
                        />
                      </b-col>
      ...
      <ModalPasienExternal 
        :dataHeader="dataHeader" 
        :isDraft="isDraft"
        @pilihPasienExternal="pilihPasienExternal" 
        @changeVar="changeVar"
      />
      <ModalStatus 
        :dataHeader="dataHeader" 
  ...
  export default {
    ...
    methods: {
      async refreshPerhitungan(){
        ...
        //tax
        //tax = tax nominal
        if(vm.dataPerhitungan.tax && !vm.dataPerhitungan.tax_persen){ //untuk draf mendapatkan persen
      ...
      async auto(){
        ...
        if(local){
          ...
          if(penjualan.status == 200 && penjualan.data.length){
            ...
            //penunjang
            if(data.penunjang){
              vm.listJualPenunjang = await data.penunjang.map(x => {return {
                ...x, 
            ...
            //fasilitas
            vm.listJualFasilitas = await data.fasilitas.map(x => {return {
              ...x, 
            ...
            //jasa
            vm.listJualJasa = await data.jasa.map(x => {return {
              ...x, 
      ...
      async getDatas() {
        ...
        this.list_asuransi = list_asuransi.status == 200 ? list_asuransi.data.map(x => { return {...x, nama_asuransi_harga: `${x.nama_asuransi} - ${x.nama_harga}` } } ) : []
        //kelas kunjungan
        let list_kelas_kunjungan = await this.$_api.post("KelasKunjungan/list");
        this.list_kelas_kunjungan = list_kelas_kunjungan.status == 200 ? list_kelas_kunjungan.data.map(x => { return {...x, nama_kelas: `${x.nama_kelas_kunjungan} - ${x.nama_tarif}`} }) : []
