<template>
  <b-modal id="modal-register" size="xl" centered title="Register Penjualan" header-bg-variant="success" header-text-variant="light">
    <div class="mx-2 mb-4">
      <b-form>
        <b-form-group label-cols-md="3">
          <template v-slot:label> Jumlah <span class="text-danger">*</span> </template>
          <b-form-input type="text" class="text-right" v-model="dataPerhitungan.nama_jumlah" disabled></b-form-input>
        </b-form-group>
        <b-form-group label-cols-md="3">
          <template v-slot:label> Potongan <span class="text-danger">*</span> </template>
          <b-form-input type="number" class="text-right" v-model="dataPerhitungan.discount"></b-form-input>
          <small class="text-right d-block mr-2 mt-1 text-danger">{{ dataPerhitungan.nama_discount }}</small>
        </b-form-group>
        <hr />
        <b-form-group label-cols-md="3">
          <template v-slot:label> SubTotal <span class="text-danger">*</span> </template>
          <b-form-input type="text" class="text-right" v-model="dataPerhitungan.nama_sub_total" disabled></b-form-input>
        </b-form-group>
        <b-form-group label-cols-md="3">
          <template v-slot:label> Tax (%) <span class="text-danger">*</span> </template>
          <b-form-input type="number" class="text-right" v-model="dataPerhitungan.tax_persen"></b-form-input>
          <small class="text-right d-block mr-2 mt-1 text-danger">{{ dataPerhitungan.nama_tax_persen }}</small>
        </b-form-group>
        <b-form-group label-cols-md="3">
          <template v-slot:label> Total <span class="text-danger">*</span> </template>
          <b-form-input type="text" class="text-right" v-model="dataPerhitungan.nama_total_penjualan" disabled></b-form-input>
        </b-form-group>
        <!-- <b-form-group label-cols-md="3">
    ...
    <b-card class="" header-tag="header" header-bg-variant="light">
      ...
      <b-table :items="listJualFasilitas" :fields="fieldFasilitas" responsive show-empty small bordered striped hover style="white-space: nowrap" class="">
    ...
    <b-card class="" header-tag="header" header-bg-variant="light">
      <template #header>
        <h5 class="mb-0" style="">
          <strong>Penunjang</strong>
        </h5>
      </template>
      <b-table :items="listJualPenunjang" :fields="fieldPenunjang" responsive show-empty small bordered striped hover style="white-space: nowrap" class="">
        <template #cell(no)="item">
          {{ item.index + 1 }}
        </template>
      </b-table>
    ...
    <b-card class="" header-tag="header" header-bg-variant="light">
      ...
      <b-table :items="listJualJasa" :fields="fieldJasa" responsive show-empty small bordered striped hover style="white-space: nowrap" class="">
        <template #cell(no)="item">
          {{ item.index + 1 }}
        </template>
      </b-table>
    ...
    <b-card class="" header-tag="header" header-bg-variant="light">
      ...
      <b-table :items="listJualBarang" :fields="fieldBarang" responsive show-empty small bordered striped hover style="white-space: nowrap" class="">
        <template #cell(no)="item">
          {{ item.index + 1 }}
        </template>
      </b-table>
...
export default {
  ...
  methods: {
    async register() {
      ...
      const vm = this;
      vm.busy = true;
      const bulk_fasilitas = vm.listJualFasilitas.map((x) => {
        return {
      ...
      const bulk_penunjang = vm.listJualPenunjang.map((x) => {
        return {
          ms_penunjang_id: x.ms_penunjang_id,
          keterangan_penjualan_penunjang: x.keterangan_penjualan_penunjang,
          harga_penjualan_penunjang: x.harga_penjualan_penunjang,
          harga_custom_penjualan_penunjang: x.harga_custom_penjualan_penunjang,
          qty_penjualan_penunjang: x.qty_penjualan_penunjang,
        };
      });
      const bulk_jasa = vm.listJualJasa.map((x) => {
        return {
      ...
      const bulk_barang = vm.listJualBarang.map((x) => {
        return {
