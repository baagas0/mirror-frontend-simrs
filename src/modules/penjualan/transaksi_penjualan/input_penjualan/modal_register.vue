<template>
  ...
  >
    ...
    <b-card class="" header-tag="header" header-bg-variant="light" >
      ...
      <b-table
        :items="listJualPenunjang"
        :fields="fieldPenunjang"
        responsive
        show-empty
        small
        bordered
    ...
    <template #modal-footer>
      ...
      <b-button 
        ...
        variant="primary" 
        @click="register()" 
        :disabled="busy || (!listJualFasilitas.length && !listJualJasa.length && !listJualBarang.length && !listJualPenunjang.length)"
      > Save </b-button>
      <b-button 
        v-if="isDraft == 1" 
        variant="warning" 
  ...
  export default {
    ...
    methods: {
      async register(){
        ...
        const bulk_penunjang = vm.listJualPenunjang.map(x => {return {
