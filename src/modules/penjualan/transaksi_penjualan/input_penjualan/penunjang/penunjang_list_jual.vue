<template>
    <div class="w-100">
      <b-card header-tag="header" class="h-100 w-100 m-0 p-0">
        <template #header>
          <h5 class="mb-0">
            <strong>Daftar Penunjang Yang Dijual</strong>
          </h5>
        </template>
            ...
            >
              ...
              <template #cell(actions)="item">
                <div v-if="listJual.length">
                  ...
                  >
                  <b-button
                    @click="$bvModal.show('modal-open-delete-penunjang'), data = item"
                    variant="danger"
                    size="sm"
                    :disabled="isDraft > 1"
      ...
      <b-modal
        id="modal-open-delete-penunjang"
        size="md"
        centered
        title="Perhatian .... !!!"
      ...
      >
        <p>Apakah anda yakin ingin menghapus penjualan penunjang ini?</p>
        <template #modal-footer>
          <b-button variant="secondary" @click="$bvModal.hide('modal-open-delete-penunjang')">
            Tidak
          </b-button>
  ...
  export default {
    ...
    methods: {
      ...
      async deleteJual(data){
        const vm = this
        const listJualNew = vm.listJual.filter(item=>{return item.ms_penunjang_id != data.item.ms_penunjang_id})
        ...
        if(vm.isDraft){
          const hapus = await this.$_api.post("penjualan_penunjang/delete", {...dataPerhitungan, penjualan_id: vm.dataHeader.penjualan_id, id: data.item.penjualan_penunjang_id});
          if(hapus.status == 200){
            vm.refreshListJual()
            vm.$store.commit("set_alert", { variant: "success", msg: "Berhasil Delete Fasilitas Penjualan", showing: true });
            vm.$bvModal.hide('modal-open-delete-penunjang')
          }else{
