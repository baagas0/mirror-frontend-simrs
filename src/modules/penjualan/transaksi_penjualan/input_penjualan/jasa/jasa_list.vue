    ...
    <b-card header-tag="header" class="h-100 w-100 m-0 p-0">
      ...
      <b-row>
        <b-col md="5" offset-md="7" class="mb-3">
          <b-form-input id="filter-input" v-model="filter.nama_jasa" type="search" placeholder="cari" size="sm"></b-form-input>
        </b-col>
      </b-row>

      <b-row>
        <b-col cols="12" md="12" lg="12">
          <b-table :items="items" :fields="fields" responsive show-empty small bordered striped hover :busy="busy" style="white-space: nowrap" class="">
...
export default {
  ...
  watch: {
    ...
    "filter.nama_jasa"(newVar, oldVar) {
      if (newVar != oldVar) {
        this.getDatas();
      }
    },
    "dataHeader.ms_harga_id"(newVar, oldVar) {
      if (newVar != oldVar) {
        if (!this.isDraft) this.$emit("changeVar", { to: "listJualJasa", data: [] });
        this.getDatas();
      }
    },
  ...
  methods: {
    async getDatas() {
      ...
      if (vm.dataHeader.ms_harga_id && vm.dataHeader.ms_tarif_id) {
        vm.busy = true;
        let master_harga_jasa = await this.$_api.post("msHargaJasa/listPerHalaman", {
          jumlah: vm.perPage,
        ...
        vm.items =
          master_harga_jasa.status == 200
            ? master_harga_jasa.data.map((x, idx) => {
                return {
                  ...x,
                  no: idx + 1,
                  harga_jasa: x.harga_jual,
                  nama_harga_jasa: vm.toNumeric(x.harga_jual, "idr"),
                  harga_jasa_custom: x.harga_jual,
                  qty_jasa: 1,
                  keterangan_penjualan_jasa: null,
                };
              })
            : [];
        vm.totalRows = parseInt(master_harga_jasa.count);
        vm.busy = false;
      } else {
        vm.items = [];
      }
