<template>
  <b-modal id="modal-form-jasa-jual" size="md" centered title="Jual Jasa" :header-bg-variant="dataForm.status_edit ? 'warning' : 'success'" header-text-variant="light">
    <b-form>
      <b-form-group label-cols-md="3">
        <template v-slot:label> Jasa <span class="text-danger">*</span> </template>
        <b-form-input type="text" disabled v-model="dataForm.nama_jasa"></b-form-input>
      </b-form-group>
      <b-form-group label-cols-md="3">
        <template v-slot:label> Jenis <span class="text-danger">*</span> </template>
        <b-form-input type="text" disabled v-model="dataForm.nama_jenis_jasa"></b-form-input>
      </b-form-group>
      <b-form-group label-cols-md="3">
        <template v-slot:label> Harga <span class="text-danger">*</span> </template>
        <b-form-input type="text" disabled v-model="dataForm.nama_harga_jasa"></b-form-input>
      </b-form-group>
      <b-form-group label-cols-md="3">
        <template v-slot:label> Harga Custom <span class="text-danger">*</span> </template>
        <b-form-input type="number" v-model="dataForm.harga_jasa_custom"></b-form-input>
        <small class="d-block ml-1 mt-1 text-danger">{{ dataForm.nama_harga_jasa_custom }}</small>
      </b-form-group>
      <b-form-group label-cols-md="3">
        <template v-slot:label> Jumlah <span class="text-danger">*</span> </template>
        <b-form-input type="number" v-model="dataForm.qty_jasa"></b-form-input>
        <small class="d-block ml-1 mt-1 text-danger">{{ dataForm.nama_qty_jasa }}</small>
      </b-form-group>
      <b-form-group label-cols-md="3">
        <template v-slot:label> Dokter <span class="text-danger">*</span> </template>
        <multiselect id="kelas" v-model="dataForm.jasa_dokter" :options="listJasaDokter" :multiple="false" :searchable="true" :close-on-select="true" :show-labels="false" label="nama_dokter" track-by="jasa_dokter_id" placeholder="dokter" size="sm"></multiselect>
      </b-form-group>
      <b-form-group label-cols-md="3">
        <template v-slot:label> Keterangan </template>
        <b-form-input type="text" v-model="dataForm.keterangan_penjualan_jasa"></b-form-input>
      </b-form-group>
    </b-form>
    <template #modal-footer>
      <b-button variant="secondary" @click="$bvModal.hide('modal-form-jasa-jual')"> Tidak </b-button>
      <div v-if="isDraft == 1">
        <b-button v-if="dataForm.update" variant="warning" @click="updateJual()" :disabled="busy || dataForm.qty_jasa == null"> Update </b-button>
        <b-button v-else variant="primary" @click="registerJual()" :disabled="busy || dataForm.qty_jasa == null"> Register </b-button>
      </div>
      <div v-else>
        <b-button v-if="dataForm.update" variant="warning" @click="updateJual()" :disabled="busy || dataForm.qty_jasa == null"> Ubah </b-button>
        <b-button v-else variant="primary" @click="registerJual()" :disabled="busy || dataForm.qty_jasa == null"> Tambah </b-button>
      </div>
    </template>
  </b-modal>
</template>
...
export default {
  ...
  watch: {
    dataForm(newVar, oldVar) {
      if (newVar != oldVar) {
        const vm = this;
        //gudang
        // axios.post(ipBackend+"jasaDokter/listJasaDokterByMsJasaId", {ms_jasa_id: newVar.ms_jasa_id}).then(x => {
        // axios.post(ipBackend+"324msDokter/list", {ms_jasa_id: newVar.ms_jasa_id}).then(x => {
        //   console.log(x)
        //   vm.listJasaDokter = x.status == 200 ? x.data.data : []
        // })
        this.$_api.post("jasaDokter/listJasaDokterByMsJasaId", { ms_jasa_id: newVar.ms_jasa_id }).then((x) => {
  ...
  methods: {
    ...
    async refreshListJual() {
      ...
      //list jual jasa
      const listJual =
        listJualJasa.status == 200
          ? await listJualJasa.data.map((x) => {
              return {
                ...x,
