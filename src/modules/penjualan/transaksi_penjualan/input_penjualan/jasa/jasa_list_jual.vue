<template>
  <div class="w-100">
          ...
          <b-table v-if="!busy" :items="listJual" :fields="fields" :current-page="currentPage" :per-page="perPage" :filter="filter" :filter-included-fields="filterOn" responsive show-empty small @filtered="onFiltered" bordered striped hover :busy="busy" style="white-space: nowrap" class="">
            ...
            <template #cell(actions)="item">
              <div v-if="listJual.length">
                ...
                <b-button @click="$bvModal.show('modal-open-delete-jasa'), (data = item)" variant="danger" size="sm" :disabled="isDraft > 1"><i class="ri-delete-bin-line"></i></b-button>
              </div>
            </template>
          </b-table>
        </b-col>
    ...
    <b-modal id="modal-open-delete-jasa" size="md" centered title="Perhatian .... !!!" header-bg-variant="warning" header-text-variant="light">
      <p>Apakah anda yakin ingin menghapus penjualan jasa ini?</p>
      <template #modal-footer>
        <b-button variant="secondary" @click="$bvModal.hide('modal-open-delete-jasa')"> Tidak </b-button>
...
export default {
  ...
  methods: {
    ...
    openEditJual(data) {
      ...
      vm.$emit("changeVar", { to: "dataForm", data: { ...data, index_jual: data.index, update: true } });
    },
    ...
    async deleteJual(data) {
      ...
      const listJualNew = vm.listJual.filter((item) => {
        return item.ms_jasa_id != data.item.ms_jasa_id;
      });
