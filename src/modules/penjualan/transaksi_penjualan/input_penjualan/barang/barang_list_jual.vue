<template>
  <div class="w-100">
    <b-card header-tag="header" class="h-100 w-100 m-0 p-0">
      ...
      <b-row>
        <b-col cols="12" md="12" lg="12">
          <b-table v-if="!busy" :items="listJual" :fields="fields" :current-page="currentPage" :per-page="perPage"
            ...
            <template #cell(actions)="item">
              <div v-if="listJual.length">
                <!-- {{ item.index }} -->
                <b-button class="mr-1" @click="openEditJual(item)" variant="warning" size="sm"
                  :disabled="isDraft > 1"><i class="ri-edit-line"></i></b-button>
                <b-button @click="$bvModal.show('modal-open-delete-barang'), data = item" variant="danger" size="sm"
                  :disabled="isDraft > 1"><i class="ri-delete-bin-line"></i></b-button>
              </div>
            </template>
          </b-table>
        </b-col>
    ...
    <b-modal id="modal-open-delete-barang" size="md" centered title="Perhatian .... !!!" header-bg-variant="warning"
      header-text-variant="light">
      <p>Apakah anda yakin ingin menghapus penjualan barang ini?</p>
      <template #modal-footer>
        <b-button variant="secondary" @click="$bvModal.hide('modal-open-delete-barang')">
          Tidak
        </b-button>
...
export default {
  ...
  methods: {
    ...
    async refreshListJual() {
      ...
      //list jual barang
      const listJual = listJualBarang.status == 200 ? await listJualBarang.data.map(x => {
        return {
          ...x,
    ...
    async deleteJual(data) {
      const vm = this
      const listJualNew = vm.listJual.filter(item => { return item.ms_barang_id != data.item.ms_barang_id })
