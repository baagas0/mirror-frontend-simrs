    ...
    <b-card header-tag="header" class="h-100 w-100 m-0 p-0">
      ...
      <b-row>
        <b-col cols="12" md="12" lg="12">
          <b-table :items="listBarang" :fields="fields" responsive show-empty small bordered striped hover :busy="busy" style="white-space: nowrap" class="">
...
export default {
  ...
  watch: {
    ...
    "dataHeader.refresh"(newVar, oldVar) {
      if (newVar != oldVar && newVar == "listBarang") {
        this.getDatas();
    ...
    "dataHeader.ms_gudang_id"(newVar, oldVar) {
      if (newVar != oldVar) {
        if (!this.isDraft) this.$emit("changeVar", { to: "listJualBarang", data: [] });
        this.getDatas();
      }
    },
  ...
  methods: {
    async getDatas() {
      ...
      if (vm.dataHeader.ms_harga_id && vm.dataHeader.ms_tarif_id && vm.dataHeader.ms_gudang_id) {
        ...
        // console.log('master_harga_barang', master_harga_barang)
        listBarang =
          master_harga_barang.status == 200
            ? master_harga_barang.data.map((x, idx) => {
                return {
                  ...
                  qty_barang: 1,
                  nama_qty_barang: 1,
                  keterangan_penjualan_barang: null,
                };
              })
            : [];
