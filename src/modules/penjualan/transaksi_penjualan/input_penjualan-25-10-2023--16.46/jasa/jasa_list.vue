    ...
    methods: {
      async getDatas() {
        ...
        if (vm.dataHeader.ms_harga_id && vm.dataHeader.ms_tarif_id) {
          ...
          vm.items = master_harga_jasa.status == 200 ? master_harga_jasa.data.map((x, idx) => { return {
            ...x,
            no: idx + 1,
      ...
      openAddJual(data){
        ...
        vm.$emit('changeVar', {to: 'dataForm', data: {...item, index: data.index}})
      },
      triggerAlert(event) {
        let vm = this;
