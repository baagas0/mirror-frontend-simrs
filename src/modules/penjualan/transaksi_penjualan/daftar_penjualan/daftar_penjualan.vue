<template>
  <div class="w-100">
            ...
            <form @submit.prevent="getPenjualanList" autocomplete="off">
              <b-row v-if="isShowFilters">
                <b-col cols="3" sm="3" md="3" lg="3">
                  ...
                  <multiselect id="kelas_kunjungan" v-model="filter.kelas_kunjungan" :options="list_kelas_kunjungan"
                    :multiple="false" :searchable="true" :close-on-select="true" :show-labels="false"
                    label="nama_kelas_kunjungan" track-by="kelas_kunjungan_id" placeholder="Kelas Kunjungan" size="sm">
                  </multiselect>
                </b-col>
                <b-col cols="3" sm="3" md="3" lg="3">
                  <label for="gudang">Gudang</label>
                  <multiselect id="gudang" v-model="filter.gudang" :options="list_gudang" :multiple="false"
                    :searchable="true" :close-on-select="true" :show-labels="false" label="nama_gudang" track-by="id"
                    placeholder="Gudang" size="sm"></multiselect>
                </b-col>
                <b-col cols="3" sm="3" md="3" lg="3">
                ...
                <b-col cols="3" sm="3" md="3" lg="3">
                  ...
                  <multiselect id="asuransi" v-model="filter.asuransi" :options="list_asuransi" :multiple="false"
                    :searchable="true" :close-on-select="true" :show-labels="false" label="nama_asuransi" track-by="id"
                    placeholder="Asuransi" size="sm"></multiselect>
                </b-col>
              </b-row>
              <b-row v-if="isShowFilters" class="mt-4">
                <b-col cols="3" sm="3" md="3" lg="3">
                  <label for="dokter">Dokter</label>
                  <multiselect id="dokter" v-model="filter.dokter" :options="list_dokter" :multiple="false"
                    :searchable="true" :close-on-select="true" :show-labels="false" label="nama_dokter"
                    track-by="ms_dokter_id" placeholder="Dokter" size="sm"></multiselect>
                </b-col>
                <b-col cols="3" sm="3" md="3" lg="3">
                ...
                <b-table :items="items" :fields="fields" responsive show-empty small bordered striped hover
                  ...
                  <template #cell(actions)="item">
                    <div class="text-nowrap">
                      <b-button variant="warning" class="mr-1" v-b-modal.modal-open-edit @click="is_data = item.item"><i
                          class="ri-edit-2-line"></i></b-button>
                      <b-button variant="danger" class=""
                        @click="is_data = item.item, $bvModal.show('modal-delete-penjualan')"><i
    ...
    <b-modal id="modal-delete-penjualan" size="md" centered title="Perhatian .... !!!" header-bg-variant="danger"
      header-text-variant="light">
      <p>Apakah anda yakin ingin Menghapus Penjualan</p>
      <template #modal-footer>
        <b-button variant="secondary" @click="$bvModal.hide('modal-delete-penjualan')">
          Tidak
        </b-button>
        <b-button variant="primary" @click="deletePenjualan()">
          Ya
        </b-button>
      </template>
    </b-modal>
...
export default {
  ...
  methods: {
    ...
    async getPenjualanList() {
      this.tableBusy = true;
      await this.$_api.post("penjualan/list", {
        jumlah: this.perPage,
