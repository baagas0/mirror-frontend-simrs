  ...
  <div class="w-100">
    <tas-base-crud ref="crud-rad_regis" :config="buildConfig" :classes="registrasi_id ? '-' : 'container-fluid'" :filterView="filterView" :hideFilter="hideFilter" :hideGlobalSearch="hideGlobalSearch" :mockSlave="false">
      ...
      <template v-slot:list-table-action="rowData">
        ...
        <button
          v-if="rowData.rowData.status == 0"
          class="btn btn-danger mr-1"
          @click="showCancellationModal(rowData.rowData)"
        ...
        <button
          ...
          @click="printLabelRadiologi(rowData.rowData)"
          title="Cetak Label Radiologi"
          v-if="rowData.rowData.status >= 0"
        >
          <i class="ri-price-tag-3-line"></i> Cetak Label
        </button>
      </template>
      ...
      <template v-slot:detail-footer="rowData">
        ...
        <div class="alert alert-custom alert-light-danger fade show mb-5" role="alert" v-if="rowData.rowData && rowData.rowData.status <= 1">
          ...
          <div class="alert-text">
            <strong>Perhatian!</strong> Registrasi ini dapat dibatalkan jika pasien tidak melanjutkan pemeriksaan.
          </div>
          <div class="d-flex gap-2 mt-3">
            <button class="btn btn-danger" @click="showCancellationModal(rowData.rowData)">
              <i class="ri-close-line"></i> Batalkan Pemeriksaan
            </button>
            <button class="btn btn-primary" v-if="rowData.rowData.status < 1"
              @click="updateStatus(rowData.rowData, 1)"
            >
              Proses
    ...
    <b-modal id="modal-use-registration" title="Update Kunjungan Radiologi" hide-footer>
      ...
      <ValidationObserver v-slot="{ handleSubmit }" v-if="dataObject">
        <form @submit.prevent="handleSubmit(useSubmitRegistration)" autocomplete="off">
          <div class="row">
            <div class="col-12">
              <s-input v-model="dataObject.registrasi_id" :key="'registrasi_id'" :id="'registrasi_id'" :item="{
                label: 'Kunjungan',
                id: 'registrasi_id',
                data: 'registrasi_id',
                type: 'lookup-radio',
      ...
      <template v-else><h5 class="text-danger">Data tidak ditemukan</h5>
        <pre>{{dataObject}}</pre></template>
    </b-modal>
        ...
        <form @submit.prevent="prosesPembatalan">
          <div class="form-group mb-3">
            ...
            <textarea
              ...
              class="form-control"
              rows="4"
              placeholder="Contoh: Pasien tidak datang, Pasien menolak pemeriksaan, dll."
              required
              minlength="10"
            ></textarea>
          ...
          <div class="d-flex justify-content-end gap-2">
            <button type="button" class="btn btn-secondary" @click="closeCancellationModal">
              <i class="ri-close-line"></i> Batal
            </button>
      ...
      <div size="A4" style="width: 100%; max-width: 300px; margin: 0 auto;">
        <div style="border: 2px solid #000; padding: 15px; background: white;">
          ...
          <div style="text-align: center; margin: 10px 0;">
            <span :class="selectedDataForPrint.is_cito ? 'badge' : 'badge-secondary'"
                  :style="selectedDataForPrint.is_cito ? 'background-color: #dc3545; color: white;' : 'background-color: #6c757d; color: white;'"
...
export default {
  ...
  computed: {
    ...
    labelDataReady() {
      return this.selectedDataForPrint &&
             this.selectedDataForPrint.rad_regis_id &&
             this.selectedDataForPrint.nama_lengkap
    }
  },
  mounted() {
    this.getStatistic()
  ...
  methods: {
    ...
    useSubmitRegistration() {
      this.$_alert.confirm('Konfirmasi', 'Apakah anda yakin ingin menggunakan kunjungan ini?').then((res) => {
        if (res.isConfirmed) {
          this.$_api.update('rad_regis', { ...this.dataObject, id: this.dataObject.rad_regis_id, is_registrasi: true }).then((res) => {
            this.$refs['crud-rad_regis'].$children[1].getData();
    ...
    updateRadRegis(data) {
      ...
      return
      this.$_api.update('rad_regis', { ...data, id: data.rad_regis_id }).then((res) => {
    ...
    updateStatus(data, status) {
      ...
      this.$_alert.confirm('Anda yakin ingin memproses?')
      .then((x) => { 
        if(x.isConfirmed) {
          this.$_api.update('rad_regis', { ...data, id: data.rad_regis_id, status }).then((res) => {
    ...
    downloadRecap() {
      this.$_api.downloadFile('rad_expertise/patient-recap/export-excel', {}, "rekap.xlsx").then((res) => {
    ...
    downloadRecapWaktuTunggu() {
      this.$_api.downloadFile('rad_expertise/waiting-time-analysis/export-excel', {}, "rekap-waktu-tunggu.xlsx").then((res) => {
    ...
    downloadRecapPembatalan() {
      this.$_api.downloadFile('rad_expertise/cancellation-analysis/export-excel', {}, "rekap-pembatalan.xlsx").then((res) => {
