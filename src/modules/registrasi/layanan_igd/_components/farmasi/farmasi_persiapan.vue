                      ...
                      <div class="row align-items-center">
                        ...
                        <div class="col-2 align-items-start">
                          <label class="checkbox checkbox-outline checkbox-danger">
                            <input type="checkbox" :name="'status_resep_detail_igd'+item.id" v-model="item.status_resep_detail_igd" :checked="item.status_resep_detail_igd"  @change="updateResepDetail(item)" />
                            <span></span>
                          </label>
                        </div>
                      </div>
...
export default {
  ...
  methods: {
    findDetailResep () {
      this.$_api.post('resep_detail_igd/list', { resep_igd_id: this.oResep.id, is_racik: 1, page: 1, limit: 99999 }).then((res) => {
        this.dataResepDetailRacik = res.data
        for (let a of this.dataResepDetailRacik) {
          for (let b of a.resep_detail_igds) {
    ...
    updateResepDetail (data) {
      this.$_api.update('resep_detail_igd', data).then((res) => {
        this.switchState('list')
        this.$emit('reloadData')
      })
    },
