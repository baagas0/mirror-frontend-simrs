  ...
  <div class="card card-custom">
    ...
    <b-modal ref="filter-modal" id="filter-modal" :centered="true" :hide-footer="true" :hide-header="true" :size="'xl'">
      ...
      <div class="modal-body">
        <div class="row">
          <div class="col-12">
            <b-form-group label="Status Registrasi" v-slot="{ ariaDescribedby }">
              <b-form-radio v-model="paramAPI.status" :aria-describedby="ariaDescribedby" name="status" value="">Tampilkan Semua</b-form-radio>
              <b-form-radio v-model="paramAPI.status" :aria-describedby="ariaDescribedby" name="status" value="Belum Pulang">Belum Pulang</b-form-radio>
              <b-form-radio v-model="paramAPI.status" :aria-describedby="ariaDescribedby" name="status" value="Sudah Pulang">Sudah Pulang</b-form-radio>
            </b-form-group>
          </div>
          <!-- <div class="col-12">
    ...
    data () {
      return {
        paramAPI: {
          ...
          // tanggal_awal: moment('2024/01/15').format('yyyy-MM-DD'),
    ...
    methods: {
      ...
      getMyAction (reset = false) {
        if (this.page < this.totalPage || reset) {
          ...
          this.loading = true
          this.$_api
            .list('/registrasi/listKunjunganIGDPerHalamanBypassUri', { ...filter, ...this.paramAPI })
            .then((res) => {
              this.loading = false
