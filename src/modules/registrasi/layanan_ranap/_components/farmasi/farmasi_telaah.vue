...
export default {
  ...
  methods: {
    onSubmit(data) {
      ...
      this.$_api[this.state]('resep_detail_rnap', this.mData).then((res) => {
    ...
    async submitData() {
      this.$_alert.confirm('Konfirmasi', 'anda yakin ingin melanjutkan?').then(async (status) => {
        if (status.isDismissed) return
          ...
          // OBAT RACIK
          let resep_racik = await this.$_api.post('resep_detail_rnap/list', { resep_rnap_id: this.oResep.id, is_racik: 1, page: 1, limit: 99999 })
          for (const rr of resep_racik.data) {
            for (let resep of rr.resep_detail_rnaps) {
              // console.log(resep)
              const id_obat = resep.final_id_obat ? resep.final_id_obat : resep.awal_id_obat
              const qty = resep.final_id_obat ? resep.final_qty : resep.awal_qty
    ...
    switchState(state, data = null) {
      if(data) {
        if (!data.id) data.id = data.id_resep_detail_rnap
        this.mData = data
      }
      this.state = state
    },
