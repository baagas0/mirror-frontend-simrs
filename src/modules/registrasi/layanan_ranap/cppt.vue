        ...
        <template v-else-if="dataRegistrasi && registrasi_id">
          <tas-base-crud ref="CRUD-cppt" list-card :classes="'s'" :config="configCppt" :hideDetail="true">
            <template v-slot:list-action="rowData">
              <button class="btn btn-warning" @click="viewAllCppt(registrasi_id)">{{ viewAllLabel }}</button>
            </template>
              ...
              <div class="card p-3 mb-3 bg-light-warning">
                ...
                <div class="d-flex flex-row justify-content-between align-items-center">
                  <div class="pointer" @click="assignData(rowData.rowData.cppt_id)">
                    <p class="mb-2">
                      <strong>{{ rowData.rowData.tanggal_cppt | parse("longDateTime") }}</strong>
                    </p>
                    <h5 class="font-weight-bolder text-warning mb-0 pb-0">{{ rowData.rowData.nama_dokter }}</h5>
                  </div>
                  <div>
                    <p style="text-align: end">
                      <strong>{{ rowData.rowData.nama_tenaga_medis }} ({{ rowData.rowData.nama_tipe_tenaga_medis }})</strong>
                    </p>
                    ...
                    <div class="d-flex justify-content-end">
                      ...
                      <span v-if="!shouldRedirectPerawatRow(rowData.rowData)" class="label label-lg label-pill label-inline mr-2 label-primary pointer" @click="$refs['CRUD-cppt'].getEvent('update', rowData.rowData)"><i class="ri-pencil-line text-white"></i> Edit</span>
                      <span v-else class="label label-lg label-pill label-inline mr-2 label-info pointer" @click="goToPerawatPage(rowData.rowData.registrasi_id)"><i class="ri-external-link-line text-white"></i> Halaman Perawat</span>
            ...
            <template v-slot:detail-footer="rowData">
              ...
              <div v-if="isPerawatPage">
                <cppt-perawat :dataRegistrasi="dataRegistrasi" :objectDataAssesmen="objectDataAssesmen" :dataAssesmen="dataAssesmen"></cppt-perawat>
              </div>
              <div v-else class="accordion accordion-toggle-arrow mb-6" id="accordionExample1">
                <div class="card">
                  <div class="card-header">
                    <div class="card-title" :class="{ collapsed: cpptTypeActive !== 'dokter' }" @click="cpptTypeActive === 'dokter' ? (cpptTypeActive = '') : (cpptTypeActive = 'dokter')">Dokter</div>
                  </div>
                ...
                <div class="card">
                  <div class="card-header">
                    <div class="card-title" :class="{ collapsed: cpptTypeActive !== 'perawat' }" @click="cpptTypeActive === 'perawat' ? (cpptTypeActive = '') : (cpptTypeActive = 'perawat')">Perawat</div>
                  </div>
          ...
          <b-card-header header-tag="div" class="p-1 bg-success" role="tab" v-b-toggle="'collapse-' + item.cppt_id" style="min-height: 0px; border-radius: 10px 10px 0px 0px">
            <div class="card-title" style="padding: 0.7rem 1rem; width: 95%">
              <div class="d-flex flex-row justify-content-between align-items-center w-100">
                <div class="">
                  <p class="mb-2">
                    <strong>{{ item.tanggal_cppt | parse("longDateTime") }}</strong>
                  </p>
                  <h5 class="font-weight-bolder text-warning mb-0 pb-0">{{ item.nama_dokter }}</h5>
                </div>
                <div class="d-flex align-items-center">
                  <p class="mb-0" style="text-align: end">
                    <strong>{{ item.nama_tenaga_medis }} ({{ item.nama_tipe_tenaga_medis }})</strong>
                  </p>
                </div>
              </div>
            </div>
...
const CPPT_TIPE_TENAGA_MEDIS_OPTIONS = [
  {
...
const getTipeTenagaMedisByCode = (code) => {
  return CPPT_TIPE_TENAGA_MEDIS_OPTIONS.find((item) => item.kode_tipe_tenaga_medis === code) || null;
};

export default {
  name: "layanan_ranap_cppt",
  props: {
    fixedKodeTipeTenagaMedis: {
      type: String,
      required: false,
      default: "",
    },
  ...
  data() {
    const fixedTipeTenagaMedis = getTipeTenagaMedisByCode(this.fixedKodeTipeTenagaMedis);
    const tipeTenagaMedisOptions = fixedTipeTenagaMedis ? CPPT_TIPE_TENAGA_MEDIS_OPTIONS.filter((item) => item.kode_tipe_tenaga_medis === this.fixedKodeTipeTenagaMedis) : CPPT_TIPE_TENAGA_MEDIS_OPTIONS.filter((item) => item.kode_tipe_tenaga_medis !== "PRW");
    ...
    return {
      ...
      showFormCppt: false,
      fixedTipeTenagaMedis,
      configCppt: {
        title: fixedTipeTenagaMedis ? `Daftar CPPT ${fixedTipeTenagaMedis.nama_tipe_tenaga_medis}` : "Daftar CPPT",
        model_api: "",
        getter: "cppt",
        setter: "cppt",
        pk_field: "cppt_id",
        ...
        fields: [
          ...
          // Tipe Tenaga Medis
          {
            ...
            methods: {
              ...
              create: {
                type: fixedTipeTenagaMedis ? "hidden" : "lookup-radio",
                value: fixedTipeTenagaMedis ? fixedTipeTenagaMedis.id : "",
                option: {
                  list_pointer: {
                    label: "nama_tipe_tenaga_medis",
                    code: "id",
                    display: ["kode_tipe_tenaga_medis", "nama_tipe_tenaga_medis"],
                    list: tipeTenagaMedisOptions,
                  },
                },
              },
              update: {
          ...
          // Nama Tenaga Medis
          {
            ...
            label: fixedTipeTenagaMedis ? fixedTipeTenagaMedis.nama_tipe_tenaga_medis : "Nama Tenaga Medis",
            placeholder: null,
            methods: {
              list: true,
              detail: true,
  ...
  computed: {
    isPerawatPage() {
      return this.fixedKodeTipeTenagaMedis === "PRW";
    },
    viewAllLabel() {
      return this.isPerawatPage ? "Lihat Semua CPPT Perawat" : "Lihat Semua CPPT";
    },
  },
  watch: {
    "$route.query": {
      ...
      handler: function (val) {
        if (val.view === "detail") {
          this.$router.push({ name: this.$route.name, query: { view: "list", id: "" } });
        }
      },
    },
  },
  ...
  methods: {
    shouldRedirectPerawatRow(row = {}) {
      return !this.isPerawatPage && (row.kode_tipe_tenaga_medis === "perawat" || row.kode_tipe_tenaga_medis === "PRW");
    },
    goToPerawatPage(registrasiId = "") {
      const query = registrasiId ? { registrasi_id: registrasiId } : {};
      this.$router.push({ name: "Layanan RInap CPPT Perawat", query });
    },
    viewAllCppt(registrasi_id) {
      ...
      let param = { registrasi_id: registrasi_id };
      if (this.fixedKodeTipeTenagaMedis) param.kode_tipe_tenaga_medis = this.fixedKodeTipeTenagaMedis;

      this.$_api.list(this.endpoint, param).then((res) => {
        console.log(res);
        ...
        this.$_api
          ...
          .then((res) => {
            if (res.data.length > 0) {
              ...
              // if (json_keperawatan.subjective) this.dataAssesmen.subjective = json_keperawatan.subjective
              // else this.dataAssesmen.subjective = {}
              // if (json_keperawatan.objective) this.dataAssesmen.objective = json_keperawatan.objective
              // else this.dataAssesmen.objective = {}
              // if (json_keperawatan.assesment) this.dataAssesmen.assesment = json_keperawatan.assesment
              // else this.dataAssesmen.assesment = {}
              // if (json_keperawatan.planning) this.dataAssesmen.planning = json_keperawatan.planning
              // else this.dataAssesmen.planning = {}
    ...
    async selectRegistrasi(e, moveToList = true) {
      ...
      if (this.fixedKodeTipeTenagaMedis) this.configCppt.filter_api.kode_tipe_tenaga_medis = this.fixedKodeTipeTenagaMedis;
      ...
      let field_tipe_tenaga_medis_id = this.configCppt.fields.filter((x) => x.id == "ms_tipe_tenaga_medis_id");
      if (field_tipe_tenaga_medis_id.length && this.fixedTipeTenagaMedis) {
        field_tipe_tenaga_medis_id[0].methods.create.value = this.fixedTipeTenagaMedis.id;
        field_tipe_tenaga_medis_id[0].methods.update.value = this.fixedTipeTenagaMedis.id;
      }
      if (this.$route.query.view !== "list" && moveToList) this.$router.push({ name: this.$route.name, query: { view: "list" } });
    ...
    deleteData(data) {
      ...
      this.$_alert.confirm("Hapus Permanen", "Data yang sudah dihapus tidak dapat dikembalikan, lanjutkan ?").then((result) => {
        if (result.isConfirmed) {
          this.$_api
            .delete(this.endpoint, { id: data.cppt_id })
            .then((res) => {
              // this.getData(this.paramAPI, true)
              let param = { limit: 1, page: 1, registrasi_id: this.registrasi_id };
              if (this.fixedKodeTipeTenagaMedis) param.kode_tipe_tenaga_medis = this.fixedKodeTipeTenagaMedis;
              this.$refs["CRUD-cppt"].$children[1].getData(param, true);
