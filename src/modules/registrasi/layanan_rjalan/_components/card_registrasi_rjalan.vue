<template>
  <div class="card card-custom">
    ...
    <div class="card-body p-3">
      ...
      <div class="offcanvas-content pr-5 mr-n5 ps infinite flex-fill"  ref="se" :class="{ 'mt-6': loading }" v-infinite-scroll="getMyAction" :infinite-scroll-disabled="loading" infinite-scroll-distance="10">
          ...
          <sequential-entrance fromBottom delay="80">
            <div v-for="(m, i) in mymoduleList" :key="i + '-my-action'" @click="selectData(m)" class="card p-3 pointer mb-3" :class="{ 'bg-light-warning': activeData  && activeData.registrasi_id !== m.registrasi_id, 'bg-light-success': activeData  && activeData.registrasi_id === m.registrasi_id }">
              <div class="d-flex flex-row justify-content-between align-items-center">
                <div>
                  <p class="text-muted mb-0 font-weight-bold font-size-sm">Poli: {{ m.nama_poliklinik || '-' }}</p>
                  ...
                  <span class="label label-inline" v-if="type === 'farmasi'" :class="{ 'label-success': m.tahap_resep > 4, 'label-warning': m.tahap_resep <= 4 }">{{ tahapResep[m.tahap_resep] || m.tahap_resep }}</span>
                </div>
    ...
    <b-modal ref="filter-modal" id="filter-modal" :centered="true" :hide-footer="true" :hide-header="true" :size="'xl'">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Filter Kunjungan Rawat Jalan</h5>
      ...
      <div class="modal-body">
        <div class="row">
          ...
          <div class="col-12">
            <s-input v-model="paramAPI.ms_dokter_id" :item="{
              label: 'Dokter',
              id: 'ms_dokter_id',
              data: 'ms_dokter_id',
              type: 'lookup-radio',
              validation: ['required'],
              value: paramAPI.ms_dokter_id,
              api: 'msDokter',
              pointer: {
                label: 'nama_dokter',
                code: 'ms_dokter_id',
              ...
              param: {},
            }" :valuee="paramAPI.ms_dokter_id" />
          </div>
          <div class="col-12">
            <s-input v-model="paramAPI.poliklinik_id" :item="{
              label: 'Poliklinik',
              id: 'poliklinik_id',
              data: 'poliklinik_id',
              type: 'lookup-radio',
              validation: ['required'],
              value: paramAPI.poliklinik_id,
              api: 'msPoliklinik',
              pointer: {
                label: 'nama_poliklinik',
                code: 'id',
                display: ['kode_poliklinik', 'nama_poliklinik']
              },
              param: {},
            }" :valuee="paramAPI.poliklinik_id" />
          </div>
          <div class="col-12">
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        endpoint: '/registrasi/listKunjunganRajalPerHalamanBypassUri',
        paramAPI: {
    ...
    methods: {
      ...
      setEndpoint (type) {
        if (type === 'general') this.endpoint = '/registrasi/listKunjunganRajalPerHalamanBypassUri'
        else if (type === 'farmasi') this.endpoint = '/registrasi/listKunjunganRajalWithResepPerHalamanBypassUri'
      },
      getMyAction (reset = false) {
        if (this.page < this.totalPage || reset) {
          ...
          this.loading = true
          this.$_api
            .list(this.endpoint, { ...filter, ...this.paramAPI })
            .then((res) => {
              this.loading = false
