  ...
  <div class="d-flex flex-column">
    <div class="container-fluid">
              ...
              <b-card class="" header-tag="header">
                <template #header>
                  <b-row class="d-flex align-items-center">
                    <b-col>
                      <h3 class="mb-0">
                        <strong><span @click="customAlert('error')">Registrasi</span> <span
                            @click="customAlert('success')">IGD</span></strong>
                      </h3>
                    </b-col>
                  </b-row>
                </template>
                <div>
                  <b-row style="row-gap: 15px; margin-bottom: 2rem;">
                    ...
                    <b-col cols="3">
                      <s-input groupClass="mb-0" v-model="fields.triage_id" :item="{
                        label: 'Triage',
                        id: 'triage_id',
                        data: 'triage_id',
                        type: 'lookup-radio',
                        ...
                        param: {
                          pasien_id: fields.pasien_id,
                          is_registrasi: false,
                        }
                      }" />
                    </b-col>
                    <b-col cols="3">
                      <s-input v-model="fields.sebab_sakit" :key="'tujuan_kunjungan'" :id="'tujuan_kunjungan'" :item="{
                        label: 'Sebab Sakit',
                        id: 'sebab_sakit',
                        data: 'sebab_sakit',
                        type: 'lookup-radio',
                        ...
                        pointer: {
                          ...
                          display: ['label'],
                          list: [
                            { label: 'KKL (kecelakaan lalu lintas)', code: 'kkl' },
                            { label: 'BKLL (Bukan kecelakaan lalu lintas)', code: 'bkkl' },
                            { label: 'KLL dan bukan kecelakaan kerja', code: 'kll' },
                            { label: 'KK (kecelakaan kerja)', code: 'kk' }
                          ]
                        },
                        param: {},
                      }" />
                  ...
                  <b-row style="row-gap: 15px;">
                    ...
                    <b-col cols="3" v-if="conditionalFields.no_rujukan.value">
                      ...
                      <b-button class="w-100 mb-2" variant="info"
                        @click="getRujukanBerdasarkanNomorKartuMulti(fields.no_asuransi_registrasi)">Cari
                        Daftar No Rujukan (No Kartu)</b-button>
                ...
                <template #footer>
                  <div class="d-flex justify-content-between">
                    <div class="d-flex align-items-center">
                      <b-button block sizes="lg" variant="success" class="mt-0 text-nowrap" v-b-modal.modal-tutup-kunjungan
                        :disabled="!fields.id" @click="tutup_kunjungan.registrasi_id = fields.id">Tutup Kunjungan</b-button>
                    ...
                    <div class="d-flex align-items-center">
                      ...
                      <b-button block sizes="lg" variant="primary" type="submit"
                        class="ml-2 mt-0 text-nowrap">
      ...
      <ModalSEP :no_asuransi_registrasi="fields.no_asuransi_registrasi" :modalSEP1="modalSEP1" :modalSEP2="modalSEP2"
        :pesertaProps="fields" :jenisKunjungan="'IGD'" @createdSEP="handleCreatedSEP" :noSEP="fields.no_sep"
        ...
        <b-col cols="12">
          <b-card class="" hide-header>
            <div>
              <b-row style="row-gap: 15px; margin-bottom: 2rem;">
                <b-col cols="12">
                  <DaftarKunjunganRegistrasi :jenisKunjungan="'IGD'" @selectKunjungan="handleSelectKunjungan"
                    :isCreatedOrUpdated="isCreatedOrUpdated" @isCreatedOrUpdated="isCreatedOrUpdated = false" />
                </b-col>
              </b-row>
            </div>
          </b-card>
...
export default {
  ...
  mounted() {
    this.fields.ms_jenis_layanan_id = '81f8af53-cd8f-4058-b990-dae7192081cc'
    this.fields.tgl_registrasi = this.dateToday
  },
  methods: {
    logg(rowData) {
    },
    ...
    resetForm() {
      ...
      this.fields = {
        ...
        sebab_sakit: '',
        triage_id: '',
        ms_jenis_layanan_id: '81f8af53-cd8f-4058-b990-dae7192081cc',
        kelas_kunjungan_id: '',
        ms_dokter_id: '',
        ms_spesialis_id: '',
        keterangan_registrasi: '',
      ...
      this.$_alert.confirm('Simpan Data', `Data registrasi rawat jalan akan ${createOrUpdate}, ingin melanjutkan?`).then((res) => {
        if (res.isConfirmed) {
          ...
          if (res.value && this.modeSubmit === this.modes.add) {
            delete param.id
            this.$_api.post(this.configAPI.submit, param).then(async (res) => {
              this.$_alert.success('Berhasil', 'Data berhasil disimpan')
    ...
    onSelectedPasien(data) {
      this.$_api.post('pasien/detailsById', {
        ...
        if (res.status === 200 && res.data.length > 0) {
          ...
          // FIND TRIAGE
          this.$_api.post('triage/list', { is_registrasi: false, pasien_id: res.data[0].pasien_id }).then((resTriage) => {

            const temp_triage_id = this.fields.triage_id
            this.fields.triage_id = ''

            if (resTriage.status === 200 && resTriage.data.length > 0) this.fields.triage_id = resTriage.data[0].id
            else if (resTriage.data.length === 0) this.fields.triage_id = this.fields.triage_id = temp_triage_id
            // else if (resTriage.data.length === 0) this.$_alert.error({}, 'Tidak dapat menemukan triage untuk pasien ini.')
          })
          .catch((err) => {
    ...
    handleSelectKunjungan(item) {
      ...
      this.fields.triage_id = real.triage_id
      this.fields.sebab_sakit = real.sebab_sakit
