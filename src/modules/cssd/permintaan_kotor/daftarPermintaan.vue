          ...
          <b-card class="" header-tag="header">
            <template #header>
              <b-row class="d-flex align-items-center">
                <b-col>
                  <h3 class="mb-0">
                    <strong>Permintaan Alat Steril</strong>
                  </h3>
                </b-col>
            ...
            <div>
              ...
              <b-row class="mb-4">
                <b-col cols="2">
                  <b-card class="text-center" bg-variant="primary" text-variant="white">
                      <h2 class="mb-0">{{ dashboard.total || 0 }}</h2>
                      <small>Total Permintaan</small>
                  </b-card>
                </b-col>
                <b-col cols="2">
                  <b-card class="text-center" bg-variant="warning" text-variant="white">
                      <h2 class="mb-0">{{ dashboard.menunggu || 0 }}</h2>
                      <small>Menunggu</small>
                  </b-card>
                </b-col>
                <b-col cols="2">
                  <b-card class="text-center" bg-variant="info" text-variant="white">
                      <h2 class="mb-0">{{ dashboard.diambil || 0 }}</h2>
                      <small>Diambil</small>
                  </b-card>
                </b-col>
                <b-col cols="2">
                  <b-card class="text-center" bg-variant="secondary" text-variant="white">
                      <h2 class="mb-0">{{ dashboard.proses_sterilisasi || 0 }}</h2>
                      <small>Proses Sterilisasi</small>
                  </b-card>
                </b-col>
                <b-col cols="2">
                  <b-card class="text-center" bg-variant="success" text-variant="white">
                      <h2 class="mb-0">{{ dashboard.selesai || 0 }}</h2>
                      <small>Selesai</small>
                  </b-card>
                </b-col>
                <b-col cols="2">
                  <b-card class="text-center" bg-variant="light" text-variant="dark">
                      <h2 class="mb-0">{{ dashboard.total_today || 0 }}</h2>
              ...
              <form @submit.prevent="getData" autocomplete="off" class="mb-4">
                <b-row style="row-gap: 10px;">
                  ...
                  <b-col cols="2">
                    <s-input v-model="dataFilter.status_permintaan" :key="'status_permintaan'" :id="'status_permintaan'" :item="{
                      label: 'Status',
                      id: 'status_permintaan',
                      data: 'status_permintaan',
                      type: 'lookup-radio',
                      validation: [''],
                      value: dataFilter.status_permintaan,
                      pointer: {
                        label: 'label',
                        code: 'code',
                        display: ['label'],
                        list: [
                          { label: 'Semua', code: '' },
                          { label: 'Menunggu', code: '1' },
                          { label: 'Diterima Unit CSSD', code: '2' },
                          { label: 'Proses Sterilisasi', code: '3' },
                          { label: 'Selesai', code: '4' },
                        ]
                      },
                      param: {},
                    }" :valuee="dataFilter.status_permintaan" />
                  </b-col>
                  <b-col cols="3">
  ...
  methods: {
    ...
    async getData() {
      ...
      try {
        ...
        const res = await vm.$_api.post('permintaan_kotor/list', vm.dataFilter)
        if (res.status == 200) {
