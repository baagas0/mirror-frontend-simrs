<template>
  <b-modal id="modal-registrasi-eklaim" title="Pilih Registrasi" size="xl" hide-footer>
    <div>
        ...
        <b-col cols="12" md="12" lg="12">
          ...
          >
            ...
            <template #cell(actions)="item">
              <b-button @click="pilih(item.item)" :disabled="!item.item.no_sep" variant="primary" size="sm">Pilih</b-button>
            </template>
          </b-table>
        </b-col>
      </b-row>
...
export default {
  ...
  watch: {
    'dataFilter.halaman': function (val) {
      const vm = this
      vm.getData()
    },
    'dataFilter.jumlah': function (val) {
      const vm = this
      vm.getData()
    },
  },
  ...
  methods: {
    async pilih(data){
      ...
      try {
        if (data && !data.no_sep) {
          vm.$_alert.error('Registrasi belum memiliki No. SEP')
          return
        }
        console.log('pilih registrasi', data)
        vm.$emit('pilihRegistrasi', data)
        vm.$bvModal.hide('modal-registrasi-eklaim')
        await new Promise(resolve => setTimeout(resolve, 1000));
      } catch (error) {
        vm.$_alert.success('Terjadi Kesalahan System')
        // console.log(error)
      } finally {
        vm.busy = false
      }
    ...
    async getData(){
      ...
      try {
        ...
        const res = await vm.$_api.post('registrasi/list', {
        ...
        console.log('registrasi/list', res)
        if(res.status == 200){
          vm.listRegistrasi = res.data.map(x=>{
            return {
              ...x,
              nama_tgl_registrasi: vm.$moment(x.tgl_registrasi).format("LL"),
            }
          })
