  ...
  <script>
    ...
    export default {
      ...
      methods: {
        async getDatas() {
          let vm = this
          vm.data.stock_opname_id = this.$store.state.stock_opname_id
          vm.data.tgl_stock_opname = moment(new Date()).format("YYYY-MM-DD HH:mm")
          vm.tgl_stock_opname_ph = moment(new Date()).format("YYYY-MM-DD HH:mm")
          let list_gudang = await axios.post(ipBackend + "/msGudang/list");
          vm.list_gudang = list_gudang.data.status == 200 ? list_gudang.data.data.map(x => { return x }) : []

          if (vm.data.stock_opname_id != null) {
            let detail = await axios.get(ipBackend + "/stockOpname/detailsById/" + vm.data.stock_opname_id);
            console.log(detail, 'ini detail');
            vm.data.ms_gudang_id = detail.data.data[0].ms_gudang_id
            vm.data.kode_stock_opname = detail.data.data[0].kode_stock_opname
            for (let i = 0; i < vm.list_gudang.length; i++) {
              let x = vm.list_gudang[i];
              if (x.id == vm.data.ms_gudang_id) {
                vm.gudang = x
        ...
        simpanHeader() {
          let vm = this
          vm.data.ms_gudang_id = vm.gudang.id
          console.log(vm.data.tgl_stock_opname, 'ini tgl');
          axios.post(ipBackend + "/stockOpname/register", {
            tgl_stock_opname: vm.data.tgl_stock_opname,
            ms_gudang_id: vm.data.ms_gudang_id            
          })
          .then((res) => {
            console.log(res, 'ini responya');
            vm.data.stock_opname_id = res.data.data.id
            vm.data.kode_stock_opname = res.data.data.kode_stock_opname
            vm.disabledDownload = false
            this.$store.commit("set_stockOpname_id", res.data.data.id);
          })
          .catch((err) => {
            console.log(err);
          });
        ...
        downloadExcel() {
          let vm = this
          let gudang_id = vm.data.ms_gudang_id
          let tgl = vm.data.tgl_stock_opname
          window.open(`${ipBackend}/stockOpname/downloadStockByGudangId?ms_gudang_id=${gudang_id}&tgl_stock_opname${tgl}`,"_blank")
        },
        uploadExcel() {
          let vm = this
          let formData = new FormData()
          formData.append("stock_opname_id", vm.data.stock_opname_id);
          formData.append("gudang_id", vm.data.ms_gudang_id);
          formData.append("file1", vm.file1)
          axios.post(ipBackend + "/stockOpname/uploadExcel", formData)
          .then((res) => {
            console.log(res, 'ini respon upload nya ui');
            vm.items = res.data.data
            vm.itemsGagal = res.data.dataGagal
            vm.itemsTidak = res.data.dataSelisih
        ...
        simpanDataStockOpname() {
          let vm = this
          axios.post(ipBackend + "/stockOpname/simpanStockOpname", {
            stock_opname_id : vm.data.stock_opname_id
          })
          .then((res) => {
            console.log(res, 'ini respon simpan data');
            if (res.data.message == "sukses" && res.data.status == 200) {
              this.$store.commit("set_stockOpname_id", null);
              vm.data.stock_opname_id = null
              vm.data.ms_gudang_id = ""
              vm.gudang = ""
