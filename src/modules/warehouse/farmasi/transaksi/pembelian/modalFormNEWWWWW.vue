    ...
    >
      <b-form>
        <div class="row">
          ...
          <div class="col-6">
            <b-form-group v-if="is_data.kode_po">
              <template v-slot:label>
                Kode PO 
              </template>
              <b-form-input
          ...
          <div class="col-6">
            <b-form-group v-if="is_data.tgl_po">
              <template v-slot:label>
                Tanggal PO 
              </template>
              <date-picker
          ...
          <div class="col-12" v-if="data.tgl_po">
            <hr>
        ...
        <!-- ========================================== -->
        <div class="card card-custom" style="border: 1px solid rgb(206, 206, 206)" v-if="status == 1">
          <div class="card-body">
            <div class="row">
              <div class="col-12">
                <!-- <hr> -->
              ...
              <div class="col-12">
                ...
                <b-table
                  ...
                  :busy="tableBusy"
                >
                  <template #cell(kode_batch)="item">
                    <input type="text" v-model="item.item.kode_batch" class="form-control">
                  </template>
                  <template #cell(nama_tgl_kadaluarsa)="item">
                    <input type="date" v-model="item.item.tgl_kadaluarsa" class="form-control">
                  </template>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      status() {
        return this.data ? this.data.status_pembelian : 1;
      },
    },
    ...
    async created() {
      //supplier
      let list_supplier = await axios.post(ipBackend + "/msSupplier/list");
      this.list_supplier = list_supplier.data.status == 200 ? list_supplier.data.data.map(x => { return x }) : []
      //gudang
      let list_gudang = await axios.post(ipBackend + "/msGudang/list");
      this.list_gudang = list_gudang.data.status == 200 ? list_gudang.data.data.map(x => { return x }) : []
      //barang
      let list_barang = await axios.post(ipBackend + "/msBarang/list");
      this.list_barang = list_barang.data.status == 200 ? list_barang.data.data.map(x => { return x }) : []
      //satuan barang
      let list_satuan_barang = await axios.post(ipBackend + "/msSatuanBarang/list");
      this.list_satuan_barang = list_satuan_barang.data.status == 200 ? list_satuan_barang.data.data.map(x => { return x }) : []
    },
    methods: {
      async openModal(){
        ...
        if (data.tgl_po) {
          this.is_data.nama_tgl_po = new Date(this.data.tgl_po)
          let no = 1

          let subPo = await this.$_api.post("subPo/getByIdPembelian", data)

          for (const item of subPo.data) {
            const barang = this.list_barang.filter((x) => {return x.ms_barang_id === item.ms_barang_id})

            if (barang.length > 0) {
              // this.is_data_sub.barang = { ...barang[0], qjb: item.total_qty_satuan_simpan }
              // this.is_data_sub.tgl_kadaluarsa = ''
              // this.is_data_sub.kode_batch = ''
              
              // this.$nextTick(() => {
              //   this.loadSub(this.is_data_sub)
              // });

              let data = {}
              data.harga_beli = barang[0].harga_pokok
              data.qty_beli = 1  
              data.qty_satuan_simpan = barang[0].qjb
              data.satuan_beli = {id: barang[0].ms_satuan_jual_id, nama_satuan: barang[0].nama_satuan_jual}
              // ========================================
              if(data.harga_beli !== null){
                data.total_harga_beli = data.qty_beli * data.harga_beli
                data.nama_total_harga_beli = this.toNumeric(data.total_harga_beli, 'idr')
              }
              if(data.qty_satuan_simpan !== null){
                data.total_qty_satuan_simpan = data.qty_beli * data.qty_satuan_simpan
                data.nama_total_qty_satuan_simpan = this.toNumeric(data.total_qty_satuan_simpan)
              }
              // ========================================
              if(data.harga_beli !== null){
                data.harga_per_satuan_simpan = data.harga_beli / data.qty_satuan_simpan
                data.nama_harga_per_satuan_simpan = this.toNumeric(data.harga_per_satuan_simpan, 'idr')
              }
              if(data.qty_beli !== null){
                data.total_qty_satuan_simpan = data.qty_beli * data.qty_satuan_simpan
                data.nama_total_qty_satuan_simpan = this.toNumeric(data.total_qty_satuan_simpan)
              }
              // ========================================
              if(data.qty_beli !== null){
                data.total_harga_beli = data.harga_beli * data.qty_beli
                data.nama_total_harga_beli = this.toNumeric(data.total_harga_beli, 'idr')
              }
              if(data.qty_satuan_simpan !== null){
                data.harga_per_satuan_simpan = data.harga_beli / data.qty_satuan_simpan
              ...
              this.$_alert.error({}, `Barang ${item.nama_barang} tidak ditemukan`)
            }
          }

          let subPembelian = await this.$_api.post("subPembelian/getByIdPembelian", {ms_pembelian_id: this.data.pembelian_id})

          // console.log('subPo', subPo)
          // console.log('is_data_sub', this.is_data_sub)
          // console.log('subPembelian', subPembelian)
        }
      },
      loadSub(dataa = {}){
        const vm = this
        ...
        // console.log('ini data form barang', this.is_data_sub.barang)
        // console.log('ini data form satuan_beli', this.is_data_sub.satuan_beli)

        // console.log('ini form ', dataa)
        // console.log('ini form barang', dataa.barang)
        // console.log('ini form satuan_beli', dataa.satuan_beli)
        let add = true
      ...
      simpan() {
        ...
        axios
          .post(ipBackend + "/pembelian/register", {...vm.is_data, id: vm.is_data.pembelian_id, sub_pembelian: vm.table_sub})
          .then((res) => {
