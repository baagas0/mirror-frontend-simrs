<template>
    <div>
      ...
      <modal-input :gudang_utama="gudang_utama" @alertFromChild="triggerAlert($event), getDatas()" />
      <modal-edit
  ...
  export default {
    ...
    methods: {
      ...
      async getDatas() {
        this.tableBusy = true;
        let gudang = await axios.get(ipBackend + "msGudang/list");
  
        this.items = gudang.data.data;
        for (let i = 0; i < this.items.length; i++) {
          let x = this.items[i];
          x.no = i + 1;
          if(x.is_utama == 0){
            x.utamanya = "Utama"
          }else if(x.is_utama == 1){
            x.utamanya = "Bukan"
          }
        }

        let gudang_utama = await axios.post(ipBackend + "msGudang/listWithParam", {is_utama : '0'});
        this.gudang_utama = gudang_utama.data.data
        console.log(this.items, 'ini gudang all');
        console.log(this.gudang_utama, 'ini gudang utama');
        this.totalRows = this.items.length;
