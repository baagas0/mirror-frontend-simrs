          ...
          <b-card header-tag="header" header-bg-variant="dark">
                    ...
                    <b-col cols="12" md="12" lg="12">
                      ...
                      >
                        <template #cell(qty_sub)="item">
                          <b-form-group>
                            <b-form-input v-model="item.item.qty_sub" type="number">
                            </b-form-input>
                          </b-form-group>
                        </template>
            ...
            <b-row>
              ...
              <b-col class="text-right">
                <b-button variant="primary" @click="simpanData">Simpan</b-button>
              </b-col>
            </b-row>
          </b-card>
        </b-col>
...
export default {
  ...
  methods: {
    ...
    async listStockByGudangId(a) {
      ...
      try {
        let res = await this.$_api.post("stock/listStockByGudangId", {
    ...
    async getDatas() {
      try {
        this.tableBusy = true;
        let { res, err } = await useGetAxios("adjustment/list");
        if (!res) {
          throw err;
        } else {
          this.items = res;
    ...
    async simpanData() {
      ...
      try {
        let res = await this.$_api.post("mutasi/createMutasi", formMutasi);
    ...
    deleteData(a, b) {
      this.items_adjustment.splice(b, 1);
    },
