...
export default {
  ...
  methods: {
    ...
    async createAdjustment() {
      ...
      try {
        let res = await this.$_api.post("adjustment/register", {
        ...
        let x = res.data;
        if (x.message === "sukses") {
          this.form_adjustment.adjustment_id = x.id;
        } else {
          throw x;
        }
      } catch (err) {
    ...
    async getListSubAdjustmentByAdjustmentId() {
      try {
        ...
        if (res.message === "sukses") {
          this.items_adjustment = res.data;
    ...
    async simpanAdjustment() {
      try {
        let res = await this.$_api.post("adjustment/simpanAdjustment", {
          adjustment_id: this.form_adjustment.adjustment_id,
        });
        if (res.message === "sukses") {
          this.triggerAlert({
