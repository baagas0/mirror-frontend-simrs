    ...
    <div class="row mb-5" v-if="summary">
      <div class="col-md-12">
        <div class="card card-frame">
          <div class="card-body">
            <h4 class="mb-4">Ringkasan BMHP & Obat</h4>
            <div class="row">
...
export default {
  ...
  data() {
    return {
      ...
      summary: null,
      config: {
        title: 'Input BMHP & Obat Pasca Operasi',
        ...
        fields: [
          ...
          // harga_satuan
          {
            ...
            placeholder: null,
            methods: {
              list: { transform: 'currency' },
              detail: { transform: 'currency' },
              create: {
  ...
  methods: {
    async loadData() {
      try {
        ...
        // Load hasil operasi
        const hasilResponse = await this.$_api.post(`hasil_operasi/detailsById`, { id: hasilOperasiId })
        this.hasilOperasi = hasilResponse.data
    ...
    async doConfirmItem(item) {
      try {
        await this.$_api.patch(`operasi_bmhp/${item.id}/confirm`)
    ...
    async doCancelItem(item) {
      try {
        await this.$_api.patch(`operasi_bmhp/${item.id}/cancel`)
